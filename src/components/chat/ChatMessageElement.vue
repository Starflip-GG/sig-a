    ...
    export default {
        ...
        computed: {
            ...
            chatGetGameLink() {
                ...
                if(
                    this.message.message.replace(/&#x2F;/g, '/').match(/(RBLXSpace\.com\/battles\/[0-9a-fA-F]{24})/) !== null ||
                    this.message.message.replace(/&#x2F;/g, '/').match(/(RBLXSpace\.com\/duels\?game=[0-9a-fA-F]{24})$/) !== null
                ) {
                    link = this.message.message.replace(/&#x2F;/g, '/').replace(/.*RBLXSpace\.com/, '');
                }
...
<style scoped>
    ...
    .chat-message-element .element-message.message-admin button.button-user span.user-username {
        background: linear-gradient(250deg, #003aff 0%, #0034aa 100%), #a6cae8;
        -webkit-background-clip: text;
        -webkit-text-fill-color: transparent;
        background-clip: text;
        text-fill-color: transparent;
