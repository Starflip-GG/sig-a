<template>
    <div class="admin-user-settings">
        ...
        <div class="settings-element element-email">
            <div class="element-name">SET EMAIL</div>
            <div class="element-input">
                <span class="icon-wrapper">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="icon-email">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            adminEmailButton() {
                const email = this.modalsData.user.local.email;

                // Validierung der E-Mail-Adresse
                const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/; // Einfaches E-Mail-Format
                if (!email || !emailRegex.test(email)) {
                    this.notificationShow({ type: 'error', message: 'The entered email address is invalid.' });
                    return;
                }
                ...
                const data = { 
                    ...
                    local: { email: email } // Korrekte Struktur für das Backend
                };

                // Anfrage an das Backend senden
                this.adminUpdateUserEmailSocket(data);
            },
            adminMuteButton() {
        ...
        watch: {
            'modalsData': {
                handler(data, oldData) {
                    ...
                    this.adminEmail = this.modalsData.user.local.email;
                },
                deep: true
            }
        },
