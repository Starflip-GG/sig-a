                ...
                <div v-if="generalRain?.active?.type === 'user'" class="type-user">
                    <div
                        ...
                        v-bind:class="[ 
                            ...
                            'avatar-' + (generalRain?.active?.creator?.rank || 'default')
                        ]"
                    >
                        <AvatarImage v-bind:image="generalRain?.active?.creator?.avatar || ''" />
                    </div>
                    <div class="user-text">
                        <div v-html="generalRain?.active?.creator?.username || 'Unknown User'" class="text-username"></div
                        >HOSTED A<span class="gradient-green">RAIN</span>
                    </div>
                </div>
            ...
            <div class="inner-info">
                <div class="info-amount">
                    ...
                    <div class="amount-value">
                        <span>{{ rainFormatValue(generalRain?.active?.amount || 0).split('.')[0] }}</span
                        >.{{ rainFormatValue(generalRain?.active?.amount || 0).split('.')[1] }}
                    </div>
                </div>
...
export default {
    ...
    computed: {
        ...
        rainGetLevelColor() {
            const level = this.generalRain?.active?.creator?.level || 0;
...
<style scoped>
    ...
    .rain-join::before {
        ...
        left: 0;
        border-radius: 15px;
        background: linear-gradient(180deg, rgba(0, 255, 194, 0) 0%, rgb(0, 119, 255) 100%);
        z-index: -1;
    }
    ...
    .rain-join button.button-claim .button-inner {
        ...
        justify-content: center;
        align-items: center;    
        background: linear-gradient(255deg, rgb(0, 119, 255) 20%, rgb(0, 150, 170) 70%);
        clip-path: polygon(3px 0, calc(100% - 3px) 0, 100% 25%, 100% 75%, calc(100% - 3px) 100%, 3px 100%, 0 75%, 0 25%);
    }
    ...
    .rain-join button.button-claim:disabled .button-inner span {
        background: linear-gradient(256.23deg, rgb(0, 119, 255) 0%, rgb(0, 150, 170) 100%);
