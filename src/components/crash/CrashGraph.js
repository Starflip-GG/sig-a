...
Graph.prototype.drawGraph = function () {
    ...
    // Setze Stil basierend auf dem Spielstatus
    ...
    // Zeitabhängige Animation für den Farbverlauf
    const time = performance.now() / 1000; // Zeit in Sekunden
    const oscillation = Math.sin(time) * 0.5 + 0.5; // Wert oszilliert zwischen 0 und 1

    // Dynamische Farben basierend auf der Zeit
    const startColor = `rgba(${Math.floor(255 * oscillation)}, ${Math.floor(
        255 * (1 - oscillation)
    )}, 255, 1)`; // Von Blau zu Gelb
    const endColor = `rgba(${Math.floor(255 * (1 - oscillation))}, 50, ${Math.floor(
        255 * oscillation
    )}, 1)`; // Von Rot zu Pink
    ...
    // Wackeln hinzufügen
    const oscillationOffset = Math.sin(performance.now() / 200) * 5; // ±5 Pixel horizontal
    ctx.translate(oscillationOffset, 0);

    // Pulsieren hinzufügen
    const scale = 1 + Math.sin(performance.now() / 500) * 0.05; // ±5 % Skala
    ...
    if (Engine.gameState === "IN_PROGRESS") {
        ctx.drawImage(this.imageObj, 0 - 90, 0 - 155);
    }
    ctx.restore();
};
