...
const actions = {
    ...
    socketConnectGeneral({ getters, dispatch }) {
        try {
            // Check if the socket exists and is already connected
            if (getters.socketGeneral?.connected === true) {
                console.log('Socket is connected, removing all listeners.');
                getters.socketGeneral.removeAllListeners();
            }
    
            // Set the authentication token if it exists
            if (getters.authToken !== null) {
                getters.socketGeneral.auth.token = getters.authToken;
            } else {
                console.warn('AuthToken is not set. Proceeding without authentication.');
            }
    
            // Disconnect and reconnect the socket
            getters.socketGeneral.disconnect().connect();
            console.log('Socket successfully restarted.');
    
            // Start listening to events
            dispatch('socketListenGeneral').catch(error => {
                console.error('Error while adding event listeners:', error);
            });
        } catch (error) {
            console.error('Error in socketConnectGeneral:', error);
        }
    },
