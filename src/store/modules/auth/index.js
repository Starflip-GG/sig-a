...
const state = {
    ...
    authDiscordToken: null // State für den Discord-Token
};
...
const getters = {
    ...
    authDiscordToken: state => state.authDiscordToken // Getter für den Discord-Token
};
...
const mutations = {
    ...
    auth_set_discord_token(state, tokenDiscord) {
        state.authDiscordToken = tokenDiscord; // Mutation für den Discord-Token
    },
};
...
const actions = {
    ...
    async authSendDiscordToken({ getters, commit, dispatch }, discordToken) {
        if (!discordToken) {
            console.error('No Discord token provided');
            return;
        }

        commit('auth_set_discord_token', discordToken);

        const payload = { method: 'link', tokenDiscord: discordToken };
        console.log('Sending payload to backend via socket:', payload);

        if (!getters.socketGeneral) {
            console.error('Socket connection is not available.');
            return;
        }

        getters.socketGeneral.emit('sendUserDiscord', payload, (response) => {
            //console.log('Response from backend:', response);

            if (response.success) {
                console.log('Discord link successful:', response.user);
                commit('auth_set_user', response.user);
                dispatch('notificationShow', { type: 'success', message: 'Discord linked successfully!' });
            } else {
                console.error('Error during Discord linking:', response.error);
