...
const actions = {
    ...
    async userSendUserDiscordSocket({ getters, commit, dispatch }, data) {
        //console.log('Sending payload to backend via socket:', data);
        ...
        getters.socketGeneral.emit('sendUserDiscord', data, (response) => {
            //console.log('Response from backend:', response);

            if (response.success) {
                //console.log('Discord link/unlink successful:', response.user);
                commit('auth_set_user', response.user);
            } else {
                //console.error('Error during Discord linking/unlinking:', response.error);
    ...
    userSendUserTipSocket({ getters, commit, dispatch }, data) {
        ...
        getters.socketGeneral.emit('sendUserTip', data, (res) => {
            if(res.success === true) {
                ...
                dispatch('notificationShow', { type: 'success', message: 'You\'ve successfully tipped ' + (Math.floor(res.transaction.amount / 10) / 100).toFixed(2) + ' RBX.' });
            } else {
                dispatch('notificationShow', res.error);
            }
