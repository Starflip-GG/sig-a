        ...
        getters.socketMines.emit('sendReveal', data, (res) => {
            if(res.success === true) {
                ...
                if(getters.soundMines === 1 && (res.game.state !== 'completed' || res.game.payout > 0)) {
                    getters.soundCoinHit.volume = getters.soundVolume;
                    getters.soundCoinHit.currentTime = 0;
                    getters.soundCoinHit.play();
                } else if(getters.soundMines === 1) {
                    getters.soundMineHit.volume = getters.soundVolume;
                    getters.soundMineHit.play();
                }
            } else {
                dispatch('notificationShow', res.error);
            }
