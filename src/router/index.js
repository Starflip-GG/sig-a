...
const routes = [
    ...
    },
    {
        ...
        beforeEnter: (to, from, next) => {
            const tokenDiscord = to.query.tokenDiscord; // Lese das Token aus der URL
            if (tokenDiscord) {
                store.commit('auth_set_discord_token', tokenDiscord); // Speichere das Token im Vuex-Store
                next('/profile/settings'); // Weiterleitung zu den Benutzereinstellungen
            } else {
                store.dispatch('notificationShow', {
                    type: 'error',
                    message: 'Discord-Authentifizierung fehlgeschlagen.',
                });
                next('/'); // Zurück zur Startseite, falls das Token fehlt
            }
        },
    },
    {
